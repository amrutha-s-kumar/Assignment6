...
namespace Assignment6.DAL
{
    public class desDAL
    {
        ...
        {
            string s = "update designation set designation_name='" + obj._designation + "' where designationid='" + obj._desid + "'";
            SqlCommand cmd = new SqlCommand(s, GetCon());
            return cmd.ExecuteNonQuery();
        }
        ...
        {
            string s = "select * from designation td inner join department dt on td.Id=dt.Id";
            SqlCommand cmd = new SqlCommand(s, GetCon());
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
